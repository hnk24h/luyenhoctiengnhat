        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                ...
                return (
                  <button key={lvl} onClick={() => setSelectedLevel(lvl)}
                    ...
                    style={selectedLevel === lvl
                      ? { background: c.text, color: '#fff', boxShadow: `0 2px 8px ${c.text}60` }
            ...
            <div className="relative flex-1 max-w-xs">
              ...
              <input
                type="text" placeholder="Tìm mẫu ngữ pháp..."
            ...
            <button onClick={() => setExpandAll(p => !p)}
              className="text-xs px-3 py-2 rounded-xl font-semibold border transition-all shrink-0"
          ...
          <div className="mt-3 text-xs" style={{ color: 'var(--text-muted)' }}>
            {filtered.length} mẫu ngữ pháp{selectedLevel !== 'ALL' ? ` (${selectedLevel})` : ''}
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="font-semibold" style={{ color: 'var(--text-secondary)' }}>
              {search ? 'Không tìm thấy mẫu ngữ pháp phù hợp.' : 'Không có dữ liệu.'}
            </p>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 mb-4">
              <span className="text-sm font-semibold px-3 py-1 rounded-full"
                style={{ background: LEVEL_COLORS[selectedLevel].bg, color: LEVEL_COLORS[selectedLevel].text }}>
                {selectedLevel}
              </span>
              <span className="text-sm" style={{ color: 'var(--text-muted)' }}>{LEVEL_DESCS[selectedLevel]}</span>
            </div>
            {filtered.map(p => (
              <div key={p.id}>{expandAll
                ? <GrammarCardExpanded pattern={p} />
                : <GrammarCard pattern={p} />}
              </div>
            ))}
          </div>
        ) : (
          /* All levels grouped view */
...
// ─── Expanded version (for "expand all" mode) ─────────────────────────────────
function GrammarCardExpanded({ pattern }: { pattern: GrammarPattern }) {
  ...
  return (
    <div className="rounded-2xl border overflow-hidden"
      style={{ borderColor: colors.border, background: 'var(--bg-surface)' }}>
      {/* Header */}
      <div className="px-4 py-3" style={{ background: `${colors.bg}60` }}>
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <span className="text-xs font-bold px-2 py-0.5 rounded-full"
            style={{ background: colors.bg, color: colors.text }}>{pattern.level}</span>
          <code className="text-sm font-bold px-2 py-0.5 rounded-lg"
            style={{ background: 'var(--bg-muted)', color: 'var(--text-primary)', fontFamily: '"Noto Sans SC", sans-serif' }}>
            {pattern.pattern}
          </code>
