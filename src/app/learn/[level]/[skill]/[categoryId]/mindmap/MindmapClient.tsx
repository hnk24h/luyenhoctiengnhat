...
export interface MindmapItem {
  id: string; japanese: string; reading: string | null;
  meaning: string; type: string;
}
...
const ITEM_ACCENT: Record<string, { border: string; glyph: string }> = {
  ...
  kanji:   { border: '#EF4444', glyph: '#F87171' },
  ...
  return (
    ...
    >
      ...
      >
        ...
        {item.reading && (
          <div style={{ fontSize: 18, color: T.textDim, marginBottom: 12, fontFamily: '"Noto Sans JP", serif' }}>{item.reading}</div>
        )}
        <div style={{ height: 1, background: T.border, margin: '12px 0' }} />
        <div style={{ fontSize: 16, color: T.text, lineHeight: 1.65, fontWeight: 500 }}>{item.meaning}</div>
      </div>
    </div>
  );
}
