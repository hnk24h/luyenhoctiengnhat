/**
 * CLI seed for basic JLPT data: levels (N5–N1) + sample exam sets for N5.
 * This is the CLI equivalent of POST /api/admin/seed.
 *
 * Usage:  npx tsx scripts/seed/seed-basic.ts
 * Or:     pnpm db:seed-basic
 */
...
async function main() {
  ...
  for (const l of levelData) {
    ...
    console.log(`  ✓ Level ${l.code}`);
  }

  // ── Sample exam sets for N5 ────────────────────────────────────────────────
  const n5 = await prisma.level.findUnique({ where: { code: 'N5' } });
  if (!n5) throw new Error('N5 level not found after seed');

  const existing = await prisma.examSet.findFirst({
    where: { levelId: n5.id, skill: 'nghe', title: '[Mẫu] Nghe hiểu N5 - Đề 1' },
  });

  if (!existing) {
    // Listening set
    const setNghe = await prisma.examSet.create({
      data: {
        levelId: n5.id, skill: 'nghe',
        title: '[Mẫu] Nghe hiểu N5 - Đề 1',
        description: 'Bộ đề nghe hiểu mẫu cho N5', timeLimit: 1800,
      },
    });
    await prisma.question.createMany({
      data: [
        { examSetId: setNghe.id, type: 'tracnghiem', content: 'Người phụ nữ đang làm gì?', options: ['Nấu ăn','Đọc sách','Xem TV','Mua sắm'], answer: 'Đọc sách', explain: 'Từ audio cho biết người phụ nữ đang đọc sách.', order: 1 },
        { examSetId: setNghe.id, type: 'tracnghiem', content: 'Hai người đang nói chuyện ở đâu?', options: ['Nhà hàng','Nhà ga','Công viên','Siêu thị'], answer: 'Nhà ga', explain: '「駅」(えき) có nghĩa là nhà ga.', order: 2 },
        { examSetId: setNghe.id, type: 'tracnghiem', content: 'Mấy giờ thì cuộc họp bắt đầu?', options: ['9:00','10:00','14:00','15:00'], answer: '10:00', order: 3 },
      ],
    });
    console.log('  ✓ Sample listening exam set N5');

    // Reading set
    const setDoc = await prisma.examSet.create({
      data: {
        levelId: n5.id, skill: 'doc',
        title: '[Mẫu] Đọc hiểu N5 - Đề 1',
    ...
    console.log('  ✓ Sample reading exam set N5');

    // Writing/kanji set
    const setViet = await prisma.examSet.create({
      data: {
        levelId: n5.id, skill: 'viet',
        title: '[Mẫu] Chữ Kanji N5 - Đề 1',
    ...
    console.log('  ✓ Sample kanji exam set N5');
  } else {
    console.log('  ℹ Sample exam sets already exist, skipping.');
  }

  console.log('\nDone seeding basic data.');
}
